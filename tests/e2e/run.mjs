...
const normalize = (s) =>
  s
    .replace(/checkmcp v\d+\.\d+\.\d+/g, 'checkmcp vX')
    .replace(/^\d+\.\d+\.\d+$/gm, 'X.Y.Z')
    .replace(/"version": "\d+\.\d+\.\d+"/g, '"version": "X.Y.Z"');
...
const CASES = [
  ...
  { name: 'sarif', args: ['--format', 'sarif', 'tests/fixtures/clean-server.mjs'], exit: 0 },
